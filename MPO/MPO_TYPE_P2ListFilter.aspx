<%@ Page Title="MPO_TYPE_P2" Language="C#" MasterPageFile="~/Site.master" AutoEventWireup="true" CodeFile="MPO_TYPE_P2ListFilter.aspx.cs" Inherits="MPO_TYPE_P2Filter" %>
...
$(".PR_TYPE,.TYPE_DEC").autocomplete({ 
 
source: function (request, response) { 
 
var column = this.element[0].attributes["data-column-id"].value; 
 
var data = MPO_TYPE_P2Service.GetKeyWordsOneColumn(column, request.term); 
...
$('#modal1').openModal();
var result = MPO_TYPE_P2Service.Search(PageIndex, PageSize, SortExpression, SortDirection, PR_TYPE,TYPE_DEC,RederTable_Pagger);
...
var TrTempplate ="<tr>";
TrTempplate +="<td class='tdPR_TYPE'>";
TrTempplate +="<a id='btnShowPopup0' target='_blank' href='MPO_TYPE_P2Web.aspx?Q="+result[key].PR_TYPE+"'";
TrTempplate +="title=''>";
TrTempplate +="<span>"+result[key].PR_TYPE+"</span>";
TrTempplate +="</a>";
TrTempplate +="<div style='display: none'>";
TrTempplate +="<input data-column-id='PR_TYPE' type='text' MaxLength='50' length='50' class='validate truncatePR_TYPE' value='"+result[key].PR_TYPE+"' style='height: unset; margin: 0px;'>";
TrTempplate +="<label class='lblSave'>Save</label>";
TrTempplate +="<label class='lblCancel'>";
TrTempplate +="Cancel</label>";
TrTempplate +="</div>";
TrTempplate +="</td>";
TrTempplate +="<td class='tdTYPE_DEC'>";
TrTempplate +="<span class=''>"+result[key].TYPE_DEC+"</span>";
TrTempplate +="<div style='display: none'>";
TrTempplate +="<input data-column-id='TYPE_DEC' type='text' MaxLength='255' length='255' class='validate truncateTYPE_DEC' value='"+result[key].TYPE_DEC+"' style='height: unset; margin: 0px;'>";
TrTempplate +="<label class='lblSave'>Save</label>";
TrTempplate +="<label class='lblCancel'>";
TrTempplate +="Cancel</label>";
TrTempplate +="</div>";
...
var result = MPO_TYPE_P2Service.SaveColumn(id, column, data);
...
<input id ="btnNew" class="waves-effect waves-light btn center" type="button" value="New" onclick="javascript: window.open('MPO_TYPE_P2Web.aspx', '_blank');" />
</div> 
</div> 
...
<table id=tbResult class="  bordered  striped "  style="display: none;" > 
  <thead> 
  <tr> 
<th   data-column-id="PR_TYPE"  onclick="Sort(this);">PR_TYPE</th>
<th   data-column-id="TYPE_DEC"  onclick="Sort(this);">TYPE_DEC</th>
</tr>
</thead>
<tbody>
</tbody>
