...
$(".PR_SOURCE,.PR_DEC").autocomplete({ 
 
source: function (request, response) { 
 
var column = this.element[0].attributes["data-column-id"].value; 
 
var data = MPO_SOURCEService.GetKeyWordsOneColumn(column, request.term); 
...
$('#modal1').openModal();
var result = MPO_SOURCEService.Search(PageIndex, PageSize, SortExpression, SortDirection, PR_SOURCE,PR_DEC,RederTable_Pagger);
...
TrTempplate +="<a id='btnShowPopup0' target='_blank' href='MPO_SOURCEWeb.aspx?Q="+result[key].PR_SOURCE+"'";
TrTempplate +="title=''>";
TrTempplate +="<span>"+result[key].PR_SOURCE+"</span>";
TrTempplate +="</a>";
TrTempplate +="<div style='display: none'>";
TrTempplate +="<input data-column-id='PR_SOURCE' type='text' MaxLength='50' length='50' class='validate truncatePR_SOURCE' value='"+result[key].PR_SOURCE+"' style='height: unset; margin: 0px;'>";
TrTempplate +="<label class='lblSave'>Save</label>";
TrTempplate +="<label class='lblCancel'>";
TrTempplate +="Cancel</label>";
TrTempplate +="</div>";
TrTempplate +="</td>";
TrTempplate +="<td class='tdPR_DEC'>";
TrTempplate +="<span class=''>"+result[key].PR_DEC+"</span>";
TrTempplate +="<div style='display: none'>";
TrTempplate +="<input data-column-id='PR_DEC' type='text' MaxLength='255' length='255' class='validate truncatePR_DEC' value='"+result[key].PR_DEC+"' style='height: unset; margin: 0px;'>";
TrTempplate +="<label class='lblSave'>Save</label>";
TrTempplate +="<label class='lblCancel'>";
TrTempplate +="Cancel</label>";
TrTempplate +="</div>";
...
var result = MPO_SOURCEService.SaveColumn(id, column, data);
...
<input id ="btnNew" class="waves-effect waves-light btn center" type="button" value="New" onclick="javascript: window.open('MPO_SOURCEWeb.aspx', '_blank');" />
</div> 
</div> 
...
<th   data-column-id="PR_DEC"  onclick="Sort(this);">PR_DEC</th>
</tr>
</thead>
<tbody>
</tbody>
