<%@ Page Title="MPO_SIZE" Language="C#" MasterPageFile="~/Site.master" AutoEventWireup="true" CodeFile="MPO_SIZEListFilter.aspx.cs" Inherits="MPO_SIZEFilter" %>
...
$(".PR_SIZE,.SIZE_DEC").autocomplete({ 
 
source: function (request, response) { 
 
var column = this.element[0].attributes["data-column-id"].value; 
 
var data = MPO_SIZEService.GetKeyWordsOneColumn(column, request.term); 
...
$('#modal1').openModal();
var result = MPO_SIZEService.Search(PageIndex, PageSize, SortExpression, SortDirection, PR_SIZE,SIZE_DEC,RederTable_Pagger);
...
TrTempplate +="<a id='btnShowPopup0' target='_blank' href='MPO_SIZEWeb.aspx?Q="+result[key].PR_SIZE+"'";
TrTempplate +="title=''>";
TrTempplate +="<span>"+result[key].PR_SIZE+"</span>";
TrTempplate +="</a>";
TrTempplate +="<div style='display: none'>";
TrTempplate +="<input data-column-id='PR_SIZE' type='text' MaxLength='20' length='20' class='validate truncatePR_SIZE' value='"+result[key].PR_SIZE+"' style='height: unset; margin: 0px;'>";
TrTempplate +="<label class='lblSave'>Save</label>";
TrTempplate +="<label class='lblCancel'>";
TrTempplate +="Cancel</label>";
TrTempplate +="</div>";
TrTempplate +="</td>";
TrTempplate +="<td class='tdSIZE_DEC'>";
TrTempplate +="<span class=''>"+result[key].SIZE_DEC+"</span>";
TrTempplate +="<div style='display: none'>";
TrTempplate +="<input data-column-id='SIZE_DEC' type='text' MaxLength='255' length='255' class='validate truncateSIZE_DEC' value='"+result[key].SIZE_DEC+"' style='height: unset; margin: 0px;'>";
TrTempplate +="<label class='lblSave'>Save</label>";
TrTempplate +="<label class='lblCancel'>";
TrTempplate +="Cancel</label>";
TrTempplate +="</div>";
...
var result = MPO_SIZEService.SaveColumn(id, column, data);
...
<input id ="btnNew" class="waves-effect waves-light btn center" type="button" value="New" onclick="javascript: window.open('MPO_SIZEWeb.aspx', '_blank');" />
</div> 
</div> 
...
<th   data-column-id="SIZE_DEC"  onclick="Sort(this);">SIZE_DEC</th>
</tr>
</thead>
<tbody>
</tbody>
