<%@ Page Title="MPO_PRODUCT_LINE" Language="C#" MasterPageFile="~/Site.master" AutoEventWireup="true" CodeFile="MPO_PRODUCT_LINEListFilter.aspx.cs" Inherits="MPO_PRODUCT_LINEFilter" %>
...
$(".PR_PRODUCT_LINE,.PRODUCT_LINE_DEC").autocomplete({ 
 
source: function (request, response) { 
 
var column = this.element[0].attributes["data-column-id"].value; 
 
var data = MPO_PRODUCT_LINEService.GetKeyWordsOneColumn(column, request.term); 
...
$('#modal1').openModal();
var result = MPO_PRODUCT_LINEService.Search(PageIndex, PageSize, SortExpression, SortDirection, PR_PRODUCT_LINE,PRODUCT_LINE_DEC,RederTable_Pagger);
...
TrTempplate +="<a id='btnShowPopup0' target='_blank' href='MPO_PRODUCT_LINEWeb.aspx?Q="+result[key].PR_PRODUCT_LINE+"'";
TrTempplate +="title=''>";
TrTempplate +="<span>"+result[key].PR_PRODUCT_LINE+"</span>";
TrTempplate +="</a>";
TrTempplate +="<div style='display: none'>";
TrTempplate +="<input data-column-id='PR_PRODUCT_LINE' type='text' MaxLength='50' length='50' class='validate truncatePR_PRODUCT_LINE' value='"+result[key].PR_PRODUCT_LINE+"' style='height: unset; margin: 0px;'>";
TrTempplate +="<label class='lblSave'>Save</label>";
TrTempplate +="<label class='lblCancel'>";
TrTempplate +="Cancel</label>";
TrTempplate +="</div>";
TrTempplate +="</td>";
TrTempplate +="<td class='tdPRODUCT_LINE_DEC'>";
TrTempplate +="<span class=''>"+result[key].PRODUCT_LINE_DEC+"</span>";
TrTempplate +="<div style='display: none'>";
TrTempplate +="<input data-column-id='PRODUCT_LINE_DEC' type='text' MaxLength='255' length='255' class='validate truncatePRODUCT_LINE_DEC' value='"+result[key].PRODUCT_LINE_DEC+"' style='height: unset; margin: 0px;'>";
TrTempplate +="<label class='lblSave'>Save</label>";
TrTempplate +="<label class='lblCancel'>";
TrTempplate +="Cancel</label>";
TrTempplate +="</div>";
...
$('.tdPRODUCT_LINE_DEC').dblclick(function () { 
...
var result = MPO_PRODUCT_LINEService.SaveColumn(id, column, data);
...
<input id ="btnNew" class="waves-effect waves-light btn center" type="button" value="New" onclick="javascript: window.open('MPO_PRODUCT_LINEWeb.aspx', '_blank');" />
</div> 
</div> 
...
<th   data-column-id="PRODUCT_LINE_DEC"  onclick="Sort(this);">PRODUCT_LINE_DEC</th>
</tr>
</thead>
<tbody>
</tbody>
