<%@ Page Title="MPO_FORMULA_AND_TEST" Language="C#" MasterPageFile="~/Site.master" AutoEventWireup="true" CodeFile="MPO_FORMULA_AND_TESTListFilter.aspx.cs" Inherits="MPO_FORMULA_AND_TESTFilter" %>
...
$(".PR_FORMULA_AND_TEST,.FORMULA_AND_TEST_DEC").autocomplete({ 
 
source: function (request, response) { 
 
var column = this.element[0].attributes["data-column-id"].value; 
 
var data = MPO_FORMULA_AND_TESTService.GetKeyWordsOneColumn(column, request.term); 
...
$('#modal1').openModal();
var result = MPO_FORMULA_AND_TESTService.Search(PageIndex, PageSize, SortExpression, SortDirection, PR_FORMULA_AND_TEST,FORMULA_AND_TEST_DEC,RederTable_Pagger);
...
TrTempplate +="<a id='btnShowPopup0' target='_blank' href='MPO_FORMULA_AND_TESTWeb.aspx?Q="+result[key].PR_FORMULA_AND_TEST+"'";
TrTempplate +="title=''>";
TrTempplate +="<span>"+result[key].PR_FORMULA_AND_TEST+"</span>";
TrTempplate +="</a>";
TrTempplate +="<div style='display: none'>";
TrTempplate +="<input data-column-id='PR_FORMULA_AND_TEST' type='text' MaxLength='50' length='50' class='validate truncatePR_FORMULA_AND_TEST' value='"+result[key].PR_FORMULA_AND_TEST+"' style='height: unset; margin: 0px;'>";
TrTempplate +="<label class='lblSave'>Save</label>";
TrTempplate +="<label class='lblCancel'>";
TrTempplate +="Cancel</label>";
TrTempplate +="</div>";
TrTempplate +="</td>";
TrTempplate +="<td class='tdFORMULA_AND_TEST_DEC'>";
TrTempplate +="<span class=''>"+result[key].FORMULA_AND_TEST_DEC+"</span>";
TrTempplate +="<div style='display: none'>";
TrTempplate +="<input data-column-id='FORMULA_AND_TEST_DEC' type='text' MaxLength='255' length='255' class='validate truncateFORMULA_AND_TEST_DEC' value='"+result[key].FORMULA_AND_TEST_DEC+"' style='height: unset; margin: 0px;'>";
TrTempplate +="<label class='lblSave'>Save</label>";
TrTempplate +="<label class='lblCancel'>";
TrTempplate +="Cancel</label>";
TrTempplate +="</div>";
...
$('.tdFORMULA_AND_TEST_DEC').dblclick(function () { 
...
var result = MPO_FORMULA_AND_TESTService.SaveColumn(id, column, data);
...
<input id ="btnNew" class="waves-effect waves-light btn center" type="button" value="New" onclick="javascript: window.open('MPO_FORMULA_AND_TESTWeb.aspx', '_blank');" />
</div> 
</div> 
...
<th   data-column-id="FORMULA_AND_TEST_DEC"  onclick="Sort(this);">FORMULA_AND_TEST_DEC</th>
</tr>
</thead>
<tbody>
</tbody>
