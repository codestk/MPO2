<%@ Page Title="MPO_BRAND" Language="C#" MasterPageFile="~/Site.master" AutoEventWireup="true" CodeFile="MPO_BRANDListFilter.aspx.cs" Inherits="MPO_BRANDFilter" %>
...
$(".PR_BRAND,.BRAND_DEC").autocomplete({ 
 
source: function (request, response) { 
 
var column = this.element[0].attributes["data-column-id"].value; 
 
var data = MPO_BRANDService.GetKeyWordsOneColumn(column, request.term); 
...
$('#modal1').openModal();
var result = MPO_BRANDService.Search(PageIndex, PageSize, SortExpression, SortDirection, PR_BRAND,BRAND_DEC,RederTable_Pagger);
...
var TrTempplate ="<tr>";
TrTempplate +="<td class='tdPR_BRAND'>";
TrTempplate +="<a id='btnShowPopup0' target='_blank' href='MPO_BRANDWeb.aspx?Q="+result[key].PR_BRAND+"'";
TrTempplate +="title=''>";
TrTempplate +="<span>"+result[key].PR_BRAND+"</span>";
TrTempplate +="</a>";
TrTempplate +="<div style='display: none'>";
TrTempplate +="<input data-column-id='PR_BRAND' type='text' MaxLength='50' length='50' class='validate truncatePR_BRAND' value='"+result[key].PR_BRAND+"' style='height: unset; margin: 0px;'>";
TrTempplate +="<label class='lblSave'>Save</label>";
TrTempplate +="<label class='lblCancel'>";
TrTempplate +="Cancel</label>";
TrTempplate +="</div>";
TrTempplate +="</td>";
TrTempplate +="<td class='tdBRAND_DEC'>";
TrTempplate +="<span class=''>"+result[key].BRAND_DEC+"</span>";
TrTempplate +="<div style='display: none'>";
TrTempplate +="<input data-column-id='BRAND_DEC' type='text' MaxLength='255' length='255' class='validate truncateBRAND_DEC' value='"+result[key].BRAND_DEC+"' style='height: unset; margin: 0px;'>";
TrTempplate +="<label class='lblSave'>Save</label>";
TrTempplate +="<label class='lblCancel'>";
TrTempplate +="Cancel</label>";
TrTempplate +="</div>";
...
var result = MPO_BRANDService.SaveColumn(id, column, data);
...
<input id ="btnNew" class="waves-effect waves-light btn center" type="button" value="New" onclick="javascript: window.open('MPO_BRANDWeb.aspx', '_blank');" />
</div> 
</div> 
...
<table id=tbResult class="  bordered  striped "  style="display: none;" > 
  <thead> 
  <tr> 
<th   data-column-id="PR_BRAND"  onclick="Sort(this);">PR_BRAND</th>
<th   data-column-id="BRAND_DEC"  onclick="Sort(this);">BRAND_DEC</th>
</tr>
</thead>
<tbody>
</tbody>
