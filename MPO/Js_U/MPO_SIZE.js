...
(function () { 
 ...
 var tag = '{column:"' + column + '",keyword:"' + keyword + '"}';
  ...
  this.Search = function (PageIndex,PageSize,SortExpression,SortDirection,PR_SIZE,SIZE_DEC,RederTable_Pagger) {
        var result;

        var tag = '{PageIndex:"'+PageIndex+'",PageSize:"'+PageSize+'",SortExpression:"'+SortExpression+'",SortDirection:"'+SortDirection+'",PR_SIZE:"' + PR_SIZE + '",SIZE_DEC:"' + SIZE_DEC + '"}';
...
this.Save = function(PR_SIZE,SIZE_DEC)
{

            var result;

            var tag = '{PR_SIZE:"' + PR_SIZE + '",SIZE_DEC:"' + SIZE_DEC + '"}';
            var F = CallServices(url + "Save", tag, false, function(msg) {
                result = msg.d;
            });
            return result;
        };//Save
this.Update = function(PR_SIZE,SIZE_DEC)
{
...
this.Delete = function(PR_SIZE,SIZE_DEC)
{
    ...
    this.Select = function (PR_SIZE) {
        var result;

 var tag = '{PR_SIZE:"'+PR_SIZE+'"}';
        var F = CallServices(url + "Select", tag, false, function (msg) {
            result = msg.d;
        });
        return result;
    };//SelectAll
}).apply(MPO_SIZEService); 
