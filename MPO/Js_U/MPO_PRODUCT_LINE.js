var MPO_PRODUCT_LINEService = {}; 
(function () { 
    var url = "MPO_PRODUCT_LINEService.asmx/"; 
 ...
 var tag = '{column:"' + column + '",keyword:"' + keyword + '"}';
  ...
  this.Search = function (PageIndex,PageSize,SortExpression,SortDirection,PR_PRODUCT_LINE,PRODUCT_LINE_DEC,RederTable_Pagger) {
        var result;

        var tag = '{PageIndex:"'+PageIndex+'",PageSize:"'+PageSize+'",SortExpression:"'+SortExpression+'",SortDirection:"'+SortDirection+'",PR_PRODUCT_LINE:"' + PR_PRODUCT_LINE + '",PRODUCT_LINE_DEC:"' + PRODUCT_LINE_DEC + '"}';
...
this.Save = function(PR_PRODUCT_LINE,PRODUCT_LINE_DEC)
{

            var result;

            var tag = '{PR_PRODUCT_LINE:"' + PR_PRODUCT_LINE + '",PRODUCT_LINE_DEC:"' + PRODUCT_LINE_DEC + '"}';
            var F = CallServices(url + "Save", tag, false, function(msg) {
                result = msg.d;
            });
            return result;
        };//Save
this.Update = function(PR_PRODUCT_LINE,PRODUCT_LINE_DEC)
{
...
this.Delete = function(PR_PRODUCT_LINE,PRODUCT_LINE_DEC)
{
    ...
    this.Select = function (PR_PRODUCT_LINE) {
        var result;

 var tag = '{PR_PRODUCT_LINE:"'+PR_PRODUCT_LINE+'"}';
        var F = CallServices(url + "Select", tag, false, function (msg) {
            result = msg.d;
        });
        return result;
    };//SelectAll
}).apply(MPO_PRODUCT_LINEService); 
